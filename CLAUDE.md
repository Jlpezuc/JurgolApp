...
- **Home tab** (`Panel`) — the landing dashboard: career stats (from `usePlayerMatchStats`), a "actividad reciente" feed derived from the player's last played matches, and two quick actions — "Buscar partido" → the **Social** tab (that's where the marketplace lives, not Partido) and "Ver equipos" → the Squads tab.
